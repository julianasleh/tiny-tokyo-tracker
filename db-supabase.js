...
(function () {
  ...
  async function allForRefresh() {
    return must(await getClient().from('cards').select('id, game, external_id').eq('status', 'owned'));
  }
  ...
  async function soldForRefresh() {
    return must(await getClient().from('cards').select('id, game, external_id').eq('status', 'sold'));
  }
  ...
  async function getAnalysis() {
    const owned = must(await getClient().from('cards')
      .select('name, set_name, rarity, condition, game, image_url, quantity, price_current, price_at_add, currency')
      .eq('status', 'owned'));
  ...
  async function wishlistForRefresh() {
    return must(await getClient().from('wishlist').select('id, game, external_id'));
  }
