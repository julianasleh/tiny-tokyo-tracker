...
(function () {
  ...
  route('GET', '/api/profile/:id', async ({ params }) => {
    try {
      const profile = await D.getProfile(params.id);
      if (!profile) return bad(404, { error: 'Profil nicht gefunden' });
      const ratings = await D.listRatings(params.id).catch(() => []);
      return ok({ profile, ratings });
    } catch (e) {
      return bad(503, { error: 'Profile noch nicht eingerichtet. Bitte supabase-community-3.sql im Supabase-SQL-Editor ausführen.', detail: String((e && e.message) || e) });
    }
  });
  route('POST', '/api/profile/:id/rate', async ({ params, body }) => {
    const stars = parseInt(body && body.stars);
    if (!(stars >= 1 && stars <= 5)) return bad(400, { error: 'Bewertung braucht 1 bis 5 Sterne' });
    const comment = (body && body.comment) ? String(body.comment).slice(0, 500) : null;
    try { await D.rateUser(params.id, stars, comment); return ok({ ok: true }); }
    catch (e) { return bad(502, { error: String((e && e.message) || e) }); }
  });
  route('DELETE', '/api/profile/:id/rate', async ({ params }) => {
    try { return ok({ ok: await D.deleteRating(params.id) }); }
    catch (e) { return bad(502, { error: String((e && e.message) || e) }); }
  });
